...
function parseOptions(options) {
    return {
        ...
        allow: options.allow || [],
        numberRecommendation: "numberRecommendation" in options ? options.numberRecommendation : 'Number',
    };
}
        ...
        return {
            UnaryExpression(node) {
                ...
                if (!operatorAllowed && options.number && node.operator === "+" && !isNumeric(node.argument)) {
                    const recommendation = `${options.numberRecommendation}(${sourceCode.getText(node.argument)})`;
            ...
            "BinaryExpression:exit"(node) {
                ...
                if (nonNumericOperand) {
                    const recommendation = `${options.numberRecommendation}(${sourceCode.getText(nonNumericOperand)})`;
